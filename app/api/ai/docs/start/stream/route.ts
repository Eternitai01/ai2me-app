...
export async function POST(request: NextRequest) {
  ...
  try {
    upstream = await fetch(`${AI_SERVICE_URL}/v1/ai/docs/start/stream`, {
      method: "POST",
      headers,
      body,
      cache: "no-cache",
  ...
  // Raw ReadableStream passthrough — do NOT await upstream.text(), that would buffer the whole stream.
