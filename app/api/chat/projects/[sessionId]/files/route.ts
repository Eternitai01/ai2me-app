...
export async function GET(
  ...
  try {
    ...
    const envApiKey = process.env.AI_SERVICE_API_KEY || process.env.AI2ME_API_KEY;
    ...
    // Get query parameters (format)
    const searchParams = request.nextUrl.searchParams;
    const format = searchParams.get("format") || "tree";

    const response = await fetch(
      `${AI_SERVICE_URL}/v1/chat/projects/${sessionId}/files?format=${format}`,
      {
        method: "GET",
        headers,
        cache: "no-cache",
    ...
    console.error("Project files API error:", err);
    return NextResponse.json(
