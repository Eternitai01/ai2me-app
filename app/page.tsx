...
export default function Home() {
  ...
  return (
    ...
    >
      {/* Theme Toggle */}
      <button style={{ position: "absolute", top: 24, right: 36, background: isLight ? "#222" : "#fff", color: isLight ? "#fff" : "#222", border: "none", borderRadius: 22, padding: "8px 18px", fontWeight: 500, cursor: "pointer", zIndex: 9999 }} onClick={() => setIsLight((x) => !x)}>
        {isLight ? "Modo oscuro" : "Modo claro"}
      </button>
      ...
      <div style={{ maxWidth: 540, fontSize: "1.25rem", textAlign: "center", marginBottom: 30, lineHeight: 1.5 }}>
        Alcanza tus metas con una cartera gestionada por IA.
        <br/>
        Invierte en piloto automático con transferencias periódicas,
        o participa manualmente según tus estrategias.
        <br/>
        Capital sujeto a riesgo.
      </div>
      <button style={{ background: fgColor, color: bgColor, borderRadius: 28, padding: "18px 38px", fontSize: "1.25rem", fontWeight: 600, marginBottom: 32, transition: "background 0.2s,color 0.2s", border: "none", cursor: "pointer", boxShadow: "0 2px 24px #0004" }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 18, marginTop: 34 }}>
        <button onClick={() => setMode("manual")} style={{ padding: "12px 32px", fontSize: "1.1rem", borderRadius: 22, background: mode === "manual" ? fgColor : "transparent", color: mode === "manual" ? bgColor : fgColor, fontWeight: 500, border: `2px solid ${fgColor}", boxShadow: mode === "manual" ? "0 2px 16px #0002" : "none", cursor: "pointer", transition: "all 0.1s" }}>
          Inversión manual
        </button>
        <button onClick={() => setMode("automated")}
          style={{ padding: "12px 32px", fontSize: "1.1rem", borderRadius: 22, background: mode === "automated" ? fgColor : "transparent", color: mode === "automated" ? bgColor : fgColor, fontWeight: 500, border: `2px solid ${fgColor}", boxShadow: mode === "automated" ? "0 2px 16px #0002" : "none", cursor: "pointer", transition: "all 0.1s" }}>
...
export default function Home() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: bgColor, color: fgColor, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", transition: "background 0.3s, color 0.3s" }}>
      {/* Theme Toggle */}
      <button style={{ position: "absolute", top: 24, right: 36, background: isLight ? "#222" : "#fff", color: isLight ? "#fff" : "#222", border: "none", borderRadius: 22, padding: "8px 18px", fontWeight: 500, cursor: "pointer", zIndex: 99 }} onClick={() => setIsLight((x) => !x)}>
        {isLight ? "Modo oscuro" : "Modo claro"}
      </button>
